...
WaterpumpSensor::WaterpumpSensor(QWidget *parent) : QWidget(parent)
{
    ...
    //QLabel *hsnLabel = new QLabel("握手次数:");
    //hsLaybout->addWidget(hsnLabel);
...
WaterpumpSensor::~WaterpumpSensor()
{
    qDebug() << "~WaterpumpSensor()";
}

void WaterpumpSensor::setEnabled(bool b)
{
    sbase->setEnabled(b);
...
bool WaterpumpSensor::getConfig(efsConfig &cfg)
{
...
bool WaterpumpSensor::setConfig(efsConfig &cfg)
{
...
bool WaterpumpSensor::cancelApply()
{
    getATcmd()->clear();
#if 0
...
QString WaterpumpSensor::parserDevAddr(const QString &text)
{
    QString data = text;
...
QString WaterpumpSensor::getDevAddr()
{
    efsConfig cfg;
    sbase->getConfig(cfg.base);
...
bool WaterpumpSensor::setDevAddr(const QString &addr)
{
...
bool WaterpumpSensor::isValidConfig(QString *err)
{
    efsConfig cfg;
...
#if 0
QWidget *WaterpumpSensor::getManualWidget()
{
    return manualOperWidget;
}

void WaterpumpSensor::setManualWidget()
{
    mainLayout->addWidget(manualOperWidget);
}
#endif
void WaterpumpSensor::setDefConfig(SensorData::sensorType sensorType)
{
    efsConfig cfg;

    cfg.base.wm = SensorData::ABP;
    cfg.base.appskey = "51c60a4d148c719923b14a8f4d44da29";
    cfg.base.nwskey = "51c60a4d148c719923b14a8f4d44da29";
...
bool WaterpumpSensor::presetIsModify(const QString &text)
{
    efsConfig cfgdata;
    efsConfig dbdata;
...
void WaterpumpSensor::connectAtCmdFinish()
{
    connect(getATcmd(), SIGNAL(finish(ATCmd::finishState)), this, SLOT(atCmdFinish(ATCmd::finishState)));
}

bool WaterpumpSensor::saveAsPreset(const QString &name)
{
    efsConfig data;
...
bool WaterpumpSensor::loadPreset(const QString &name)
{
    efsConfig data;
...
bool WaterpumpSensor::installEventRecord(const QString &prefix)
{
...
#if 0
void WaterpumpSensor::vpgainBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::cpgainBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::cngainBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::cpoffsetBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::cnoffsetBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::enterAtModeBtnSlot()
{
    if (!getATcmd()->isIdle()) {
...
void WaterpumpSensor::atCmdFinish(ATCmd::finishState state)
{
    if (!inManualOperation) {
        return ;
    }
